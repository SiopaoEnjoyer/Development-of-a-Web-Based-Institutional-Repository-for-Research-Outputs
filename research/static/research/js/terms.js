...
(function() {
    ...
    const handlers = {
        sectionHeaders: new Map(),
        privacyHeaders: new Map()
    };
    ...
    function initialize() {
        ...
        sectionHeaders.forEach(header => {
            ...
            // Create handlers
            const clickHandler = () => toggleSection(header);
            const keyHandler = (e) => {
            ...
            // Store for cleanup
            handlers.sectionHeaders.set(header, { clickHandler, keyHandler });
        });
        ...
        privacySectionHeaders.forEach(header => {
            ...
            // Create handlers
            const clickHandler = () => togglePrivacySection(header);
            const keyHandler = (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    togglePrivacySection(header);
                }
            };
            ...
            // Store for cleanup
            handlers.privacyHeaders.set(header, { clickHandler, keyHandler });
        });
     ...
     */
    function cleanup() {
        // Remove Terms section handlers
        handlers.sectionHeaders.forEach((handlerObj, header) => {
            header.removeEventListener('click', handlerObj.clickHandler);
            header.removeEventListener('keydown', handlerObj.keyHandler);
        });
        handlers.sectionHeaders.clear();
        
        // Remove Privacy section handlers
        handlers.privacyHeaders.forEach((handlerObj, header) => {
            header.removeEventListener('click', handlerObj.clickHandler);
            header.removeEventListener('keydown', handlerObj.keyHandler);
        });
        handlers.privacyHeaders.clear();
    }
